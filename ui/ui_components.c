...
{
    xassert(ui_ctx->sprite_atlas, "SpriteAtlas needs to be loaded before calling `ui_widget_animated_label`");
    D_Animation animation_data = ui_ctx->sprite_atlas->animations[animation];
    ...
    ui_create_with_key(key, CutSideTop, animation_rect.h)
    {
        ...
        {
            animator->started_at = ui_ctx->update_t;
        }
        ...
        animator->updated_at = ui_ctx->update_t;

        d_sprite(ui_ctx->sprite_atlas, animation_data.sprite_start_index + current_frame, rect, vec2_one(), ANCHOR_C_C, ColorInvisible);
