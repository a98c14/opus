...
{
    Theme* t = arena_push_struct_zero(arena, Theme);
    t->p0    = vec2(0, 0);
    t->p2    = vec2(4, 4);
    ...
    t->rect_view    = (StyleRect){
           .softness = 2, .color = color_to_vec4(ColorWhite100), .border_radius = {.bl = 3, .br = 3, .tr = 0, .tl = 0}
    };
    t->rect_header = (StyleRect){
        .border_thickness = 0, .softness = 2, .color = color_to_vec4(ColorSlate700), .border_color = color_to_vec4(ColorInvisible), .border_radius = {.bl = 0, .br = 0, .tr = 3, .tl = 3}
    };
